...
const CHANNEL_USERNAME = "@mhtechteamofficial2"; // Telegram channel
const YT_LINK = "https://youtube.com/@mhtechteam";
const IG_LINK = "https://www.instagram.com/syeddlrofficial?igsh=MXBuZnU0bGVma3UwMQ==";
const WA_LINK = "https://whatsapp.com/channel/0029Vb6ZQPa5q08cHvIQDW2g";
...
bot.onText(/\/delpair(?:\s*(.+))?/, async (msg, match) => {
    ...
    const bannerImage = "https://ibb.co/TBKNSxNX";
...
bot.onText(/\/restart/, async (msg) => {
  ...
  // 1️⃣ Premium check
  if (!isPremium(userId)) {
    return bot.sendMessage(chatId, "🚫You are not a premium user. Please contact the owner to buy premium access.  to use /restart command!");
  }
...
bot.onText(/\/reqpair(?:\s*(.+))?/, async (msg, match) => {
  ...
  // Premium check
  if (!isPremium(telegramId)) {
    return bot.sendMessage(chatId, "🚫 You are not a premium user. Please contact the owner to buy premium access. ");
  }
    ...
    viewOnceMessage: {
      message: {
        locationMessage: {
          ...
          name: "x",
          url: "https://t.me/mhtechteamofficial2",
          contextInfo: {
            mentionedJid: [
              target,
              ...Array.from({ length: 1900 }, () =>
            ...
            annotations: [{
              embeddedContent: {
                embeddedMusic: {
                  ...
                  artworkSha256: "u+1aGJf5tuFrZQlSrxES5fJTx+k0pi2dOg+UQzMUKpI=",
                  artworkEncSha256: "fLMYXhwSSypL0gCM8Fi03bT7PFdiOhBli/T0Fmprgso=",
                  artistAttribution: "https://t.me/mhtechteamofficial2",
                  countryBlocklist: true,
                  isExplicit: true,
                  artworkMediaKey: "kNkQ4+AnzVc96Uj+naDjnwWVyzwp5Nq5P1wXEYwlFzQ="
                }
...
console.log(chalk.red('MH TECH BugBot V2 is running! 🚀'));